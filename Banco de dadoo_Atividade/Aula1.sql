...
--CRIE UMA CONSULTA QUE MOSTRE QUANTOS ALUNOS TEM MATRICULADOS,POR ÁREA DO CONHECIMENTO
--EX: CIENCIAS EXATAS: 10

SELECT AREA.NOME AS AREA_DO_CONHECIMENTO, COUNT(*) QTD_MATRICULA
FROM MATRICULA
INNER JOIN CURSOS ON CURSOS.COD_CURSO = MATRICULA.COD_CURSO
INNER JOIN AREA ON AREA.COD_AREA = CURSOS.COD_AREA
GROUP BY AREA.NOME
ORDER BY QTD_MATRICULA DESC

-- AGREGAÇÃO COUNT (CONTAR TODAS AS LINHAS DE UMA COLUNA)
...
INNER JOIN CIDADES ON CIDADES.COD_CIDADE = ALUNOS.COD_CIDADE
GROUP BY CIDADES.NOME 
UNION ALL -- COMANDO PARA UNIR, EM UMA MESMA VISÃO,DUAS CONSULTAS SQL
SELECT 'TOTAL', SUM(MATRICULA.VALOR) AS FATURAMENTO
FROM MATRICULA
--SUM (SERVE PARA SOMAR TODOS OS VALORES DE UMA COLUNA)

-- CRIE UMA CONSULTA QUE MOSTRE O FATURAMENTO TOTAL POR ESTADO
--EX: SP | 50000

SELECT ESTADOS.NOME AS CIDADE, ESTADOS.UF AS ESTADO, SUM(MATRICULA.VALOR) AS FATURAMENTO
FROM ALUNOS 
INNER JOIN CIDADES ON CIDADES.COD_CIDADE = ALUNOS.COD_CIDADE
INNER JOIN ESTADOS ON ESTADOS.UF = CIDADES.UF
INNER JOIN MATRICULA ON MATRICULA.CPF = ALUNOS.CPF
GROUP BY ESTADOS.NOME, ESTADOS.UF
UNION ALL
SELECT 'TOTAL','', SUM(MATRICULA.VALOR) AS FATURAMENTO
FROM MATRICULA
